...
GO
ALTER TABLE [dbo].[TaxRate] DROP CONSTRAINT [DF__TaxRate__Medicar__0A9D95DB];
...
GO
PRINT N'Altering Procedure [dbo].[spGetBankAccountBalances]...';


GO
ALTER PROCEDURE [dbo].[spGetBankAccountBalances]
	@date Date 
AS
begin
	set nocount on;
	select da.Id, da.CostAccountID, da.Description AccountName, da.Bank
,(isnull(credits.deposits,0)) -(isnull(debits.withdrawals,0)) Balance, 0 display
from dbo.[DebitAccount] da

left join 
		(SELECT SUM(amount) deposits, DebitID id 
		from dbo.[Transaction] 
		where DatePaid <=@date
		group by DebitID
	) credits on credits.ID = da.CostAccountID 

left join 
		(SELECT SUM(amount) withdrawals, CreditID id 
		from dbo.[Transaction] 
		where DatePaid <=@date
		group by CreditID
	) debits on debits.ID = da.CostAccountID 


UNION

select ca.id, ca.CostAccountID, ca.Description, ca.Bank,   isnull(credits.credit,0) - isnull(debits.debit,0) Balance  , 1 display
from dbo.[CreditAccount] ca

left join (
		SELECT SUM(Amount) credit, DebitID id 
		from dbo.[Transaction] 
		where DatePaid <=@date
		group by DebitID
	) credits on credits.ID = ca.CostAccountID 

left join (
		SELECT SUM(Amount) debit, CreditID id 
		from dbo.[Transaction] 
		where DatePaid <=@date
		group by CreditID
	) debits on debits.ID = ca.CostAccountID

union

select ls.id, ls.CostAccountID, ls.Description, ls.Bank, isnull(c.credit,0) - isnull(d.debit,0) Balance  , 2 display

from dbo.[LoanAccount] ls

left join (SELECT sum(isnull(amount,0)) credit, DebitID from dbo.[Transaction] where DatePaid <=@date group by DebitID) c on c.DebitID = ls.CostAccountID
left join (SELECT SUM(isnull(amount,0)) debit, creditid from  dbo.[Transaction] where DatePaid <=@date group by creditid)d on d.CreditID = ls.CostAccountID
