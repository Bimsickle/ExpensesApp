...
GO
ALTER TABLE [dbo].[TaxRate] DROP CONSTRAINT [DF__TaxRate__Medicar__628FA481];


GO
PRINT N'Starting rebuilding table [dbo].[WeekDates]...';
...
CREATE TABLE [dbo].[tmp_ms_xx_WeekDates] (
...
IF EXISTS (SELECT TOP 1 1 
           FROM   [dbo].[WeekDates])
    BEGIN
        SET IDENTITY_INSERT [dbo].[tmp_ms_xx_WeekDates] ON;
        INSERT INTO [dbo].[tmp_ms_xx_WeekDates] ([Id], [StartWeek], [EndWeek])
        SELECT   [Id],
                 [StartWeek],
                 [EndWeek]
        FROM     [dbo].[WeekDates]
        ORDER BY [Id] ASC;
        SET IDENTITY_INSERT [dbo].[tmp_ms_xx_WeekDates] OFF;
    END

DROP TABLE [dbo].[WeekDates];

EXECUTE sp_rename N'[dbo].[tmp_ms_xx_WeekDates]', N'WeekDates';
...
GO
PRINT N'Refreshing Procedure [dbo].[AddWeeks]...';


GO
EXECUTE sp_refreshsqlmodule N'[dbo].[AddWeeks]';
...
begin
    DECLARE @StartDate Date = cast (DATEADD(wk, DATEDIFF(wk,0,DATEADD(week,-12,GETDATE())), 0) as date)
