...
GO
ALTER TABLE [dbo].[TaxRate] DROP CONSTRAINT [DF__TaxRate__Medicar__08B54D69];
...
left join (SELECT sum(isnull(amount,0)) debit, DebitID from dbo.[Transaction] group by DebitID) d on d.DebitID = ls.CostAccountID
left join (SELECT SUM(isnull(amount,0)) credit, creditid from  dbo.[Transaction] group by creditid)c on c.CreditID = ls.CostAccountID
