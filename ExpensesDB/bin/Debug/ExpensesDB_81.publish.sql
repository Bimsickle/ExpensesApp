...
GO
ALTER TABLE [dbo].[TaxRate] DROP CONSTRAINT [DF__TaxRate__Medicar__09A971A2];
...
left join (SELECT sum(isnull(amount,0)) credit, DebitID from dbo.[Transaction] group by DebitID) c on c.DebitID = ls.CostAccountID
left join (SELECT SUM(isnull(amount,0)) debit, creditid from  dbo.[Transaction] group by creditid)d on d.CreditID = ls.CostAccountID
