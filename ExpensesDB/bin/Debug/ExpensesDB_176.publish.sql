...
GO
ALTER TABLE [dbo].[TaxRate] DROP CONSTRAINT [DF__TaxRate__Medicar__7E02B4CC];
...
GO
PRINT N'Creating Procedure [dbo].[spGetIncomeAccounts]...';
...
begin
	...
	SELECT i.Id, i.Active, i.Description, i.StartDate, i.EndDate, i.SalaryRate, i.SalaryCycleID, i.BankAccountID , c.LastPayDate, c.LastPayAmount
	FROM [dbo].[Income] i 

	LEFT JOIN ( SELECT a.CreditID, a.DatePaid LastPayDate, Sum(b.Amount) LastPayAmount
			from ( SELECT max(DatePaid) DatePaid, CreditID from [dbo].[Transaction]  group by CreditID) a
