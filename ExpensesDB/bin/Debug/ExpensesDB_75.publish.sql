...
GO
ALTER TABLE [dbo].[TaxRate] DROP CONSTRAINT [DF__TaxRate__Medicar__05D8E0BE];
...
begin
	set nocount on;
	select da.Id, da.CostAccountID, da.Description AccountName, da.Bank, isnull(credits.deposits,0) - isnull(debits.withdrawals,0) Balance , 0 display
from dbo.[DebitAccount] da

left join 
		(SELECT SUM(amount) deposits, DebitID id, DatePaid 
		from dbo.[Transaction] 
		group by DebitID, DatePaid
	) credits on credits.ID = da.CostAccountID and credits.DatePaid <=@date

left join 
		(SELECT SUM(amount) withdrawals, CreditID id, DatePaid 
		from dbo.[Transaction] 
		group by CreditID, DatePaid
	) debits on debits.ID = da.CostAccountID and debits.DatePaid <=@date

UNION

select ca.id, ca.CostAccountID, ca.Description, ca.Bank,  sum( isnull(credits.credit,0) - isnull(debits.debit,0)) Balance  , 1 display
from dbo.[CreditAccount] ca

left join (
		SELECT SUM(Amount) credit, DatePaid, DebitID id 
		from dbo.[Transaction] 
		group by DebitID, DatePaid
	) credits on credits.ID = ca.CostAccountID and credits.DatePaid <=@date

left join (
		SELECT SUM(Amount) debit, DatePaid, CreditID id 
		from dbo.[Transaction] 
		group by CreditID, datepaid
	) debits on debits.ID = ca.CostAccountID and credits.DatePaid <=@date
group by ca.id, ca.CostAccountID, ca.Description, ca.Bank, ca.Limit
